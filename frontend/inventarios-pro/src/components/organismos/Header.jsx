...
import { UserAuth, v,DesplegableUser, useAuthStore } from "../../index";
...
export function Header({ stateConfig }) {
  ...
  //const { user } = UserAuth();
  const { user = { email: "usuario@falso.com" } } = UserAuth();
    ...
    <Container>
      ...
      <Datauser onClick={stateConfig.setState}>
        ...
        <BtnCircular
          ...
          translatey="-12px"
        />
        <span className="nombre">{user ? user.email : 'Invitado'}</span>
        {stateConfig.state && (
